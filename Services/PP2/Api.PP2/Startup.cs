    ...
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
           ...
           .AddJwtBearer(jwtOptions =>
           {
            ...
            services.AddDbContextPool<PlanningPortalContext>((provider, builder) =>
            {
                HttpContext context = provider.GetRequiredService<IHttpContextAccessor>().HttpContext;

                var country = string.Empty;

                // Get the Country Header
                if (context != null)
                {
                    country = context.Request.Headers.TryGetValue(ApplicationConstants.Country, out var countries)
                    ? countries.Single() : string.Empty;
                }

                // Deafult country for DevOps Build Pipeline - Database Migration
                if (context == null && string.IsNullOrEmpty(country))
                {
                ...
                var connectionString = GetConnectionString(country, keyvaultObject);
            ...
            services.AddSwaggerGen(c =>
            {
                ...
                {
                    Version = "v1",
                    Title = "Terra Quest PP2 API",
                    Description = "This API holds business logic for accessing the tenants",
                });
        ...
        public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
        {
            ...
            app.UseSwaggerUI(c =>
            {
                c.SwaggerEndpoint("../swagger/v1/swagger.json", "Terra Quest PP2 API");
            });
            app.UseHealthChecks(healthCheckPath, new HealthCheckOptions
            {
                Predicate = _ => true,
                ResponseWriter = CustomHealthCheckResponseWriterAsync
            });
        }
        ...
        private static Task CustomHealthCheckResponseWriterAsync(HttpContext context, HealthReport healthReport)
        {
            context.Response.ContentType = "application/json";
            ...
            var result = JsonConvert.SerializeObject(new
            {
                ...
                {
                    key = e.Key,
                    value = $"status: {e.Value.Status.ToString()}. {e.Value.Description}"
                }),
                duration = $"{healthReport.TotalDuration.TotalMilliseconds} ms"
            });
